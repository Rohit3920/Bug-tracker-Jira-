...
export const TicketDataByProjectId = async function (projectId) {
    try {
        const response = await axios.get(`http://localhost:5000/project-ticket/${projectId}`);
        console.log('Fetched ticket data by project ID:', response.data);
        return response.data;
    } catch (error) {
        console.error('Error fetching ticket data by project ID:', error);
        throw new Error('Failed to load tickets. Please ensure your backend server is running and the API endpoint is correct.');
    }
}
